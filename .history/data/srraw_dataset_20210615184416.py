...
class SRRAWDataset(BaseDataset):
	def __init__(self, opt, split='train', dataset_name='SRRAW'):
		...
		if split == 'train':
			...
			self.raw_dirs, self.dslr_dirs, self.names = self._get_image_dir(self.root, 'test_crop', flag='00005', flag_out='00001')
			# self.raw_dirs, self.dslr_dirs, self.names = self._get_image_dir(self.root, 'test_crop', flag='00007', flag_out='00003')
			self.out_wbs = self._get_wb(self.raw_dirs, 'test_crop')
	...
	def _getitem_train(self, idx):
		...
		# print_numpy(raw)
		# print_numpy(raw_demosaic)
		# print_numpy(cropped_rgb)
		return {'raw': raw,
	...
	def _get_image_dir(self, dataroot, type='train', flag='*', flag_out='*'):
		raw_dirs = []
		dslr_dirs = [] # input_x4_raw target_x4_rgb
		image_names = sorted(glob.glob(dataroot + '/' + type + '/' + '00*'))
		for image_name in image_names:
			if type is not 'test_crop':
				raw_dir = sorted(glob.glob(image_name + '/input_x4_raw/' + flag + '.npy'))
				dslr_dir = sorted(glob.glob(image_name + '/target_x4_rgb/' + flag + '.png'))
			else:
				raw_dir = sorted(glob.glob(image_name + '/input_raw/' + flag + '.npy'))
				dslr_dir = sorted(glob.glob(image_name + '/target_rgb/' + flag_out + '.png'))
			# print(raw_dir)
		...
		for i, raw_name in enumerate(raw_dirs):
			...
			dslr_name_sp = dslr_dirs[i].split('/')
			# assert raw_name_sp[-3] + raw_name_sp[-1][:-4] == \
			# 		dslr_name_sp[-3] + dslr_name_sp[-1][:-4]
	...
	def _get_wb(self, raw_dirs, type):
		out_wbs = []
		for i, raw_name in enumerate(raw_dirs):
			if type is not 'test_crop':
				raw_name_sp = raw_name.split('/input_x4_raw/')
			else:
				raw_name_sp = raw_name.split('/input_raw/')
			key = raw_name_sp[1][:-4]		
	...
	def _crop_pair(self, raw, image, croph, cropw, tol=0, raw_tol=0, ratio=4, 
		...
		# print(raw.shape[:2], image.shape[1:])
...
# 	input_raw_img_orig = np.load('00005.npy')
# 	input_raw = reshape_back_raw(input_raw_img_orig)
# 	print_numpy(input_raw)
 
# 	raw_demosaic = colour_demosaicing.demosaicing_CFA_Bayer_bilinear(input_raw, pattern='RGGB') # RGGB

# 	raw_demosaic = np.power(raw_demosaic, 1/2.2)
# 	print_numpy(raw_demosaic)
# 	imageio.imwrite('00005.png', np.uint8(clipped(raw_demosaic)*255))
