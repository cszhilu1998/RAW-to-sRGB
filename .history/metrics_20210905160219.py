...
if __name__ == '__main__':
	...
	for file in files:
		...
		for name in tqdm(image_names): 
			...
			warp_gt = cv2.imread(file + '/warp_gt/' + name + '.png')[..., ::-1] # [32:-32,32:-32, ::-1]
			warp_gt_mask = cv2.imread(file + '/warp_gt_mask/' + name + '.png')[..., ::-1] / 255.0
		
			metrics[i, 0:4] = calc_metrics(output, test_canon[i], loss_fn_alex_v1)
			metrics[i, 4:8] = calc_metrics(output*warp_test_mask[i], warp_test_canon[i], loss_fn_alex_v1)
			metrics[i, 8:12] = calc_metrics(output*warp_gt_mask, warp_gt_canon, loss_fn_alex_v1)

			i = i + 1
		
		f.write('\n file: %s \n mean: %s \n' % (file, np.mean(metrics, axis=0)))
		print('\n file: %s \n mean: %s \n' % (file, np.mean(metrics, axis=0)))
