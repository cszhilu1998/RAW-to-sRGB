...
class ZRRGANModel(BaseModel):
	...
	def __init__(self, opt):
		...
		isp = ispjoint_model.LiteISPGAN(opt)
		self.netLiteISPGAN= N.init_net(isp, opt.init_type, opt.init_gain, opt.gpu_ids)
		...
		if self.opt.isTrain:
			...
			self.optimizer_D = optim.Adam(self.netDiscriminator.parameters(),
										  ...
										  betas=(opt.beta1, opt.beta2),
										  weight_decay=opt.weight_decay)
			self.optimizers = [self.optimizer_LiteISPGAN, self.optimizer_GCMModel, self.optimizer_D]
