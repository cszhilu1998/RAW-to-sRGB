...
import torch.nn.functional.interpolate as interpolate


# Joint training of SRResNet and GCM model for Zurich RAW to RGB (ZRR) dataset
	...
	def forward(self):
		...
		if self.isTrain:
			...
			flow = self.get_flow(interpolate(input=self.data_out, size=(2*H,2*W), mode='bilinear', align_corners=True), 
								 interpolate(input=self.data_dslr, size=(2*H,2*W), mode='bilinear', align_corners=True), 
								 self.netPWCNET)
