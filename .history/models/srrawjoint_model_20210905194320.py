...
class SRRAWJOINTModel(BaseModel):
	...
	def forward(self):
		...
		if self.opt.scale == 1:
			...
			self.down_dslr_warp = nn.functional.interpolate(input=self.dslr_warp, size=(2*H, 2*W), 
								mode='bilinear', align_corners=True)
			self.down_dslr_mask = nn.functional.interpolate(input=self.dslr_mask, size=(2*H, 2*W), 
										mode='bilinear', align_corners=True)
