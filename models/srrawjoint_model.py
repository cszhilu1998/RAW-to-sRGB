...
# Joint training of SRResNet and GCM model for SR-RAW dataset
...
class SRRAWJOINTModel(BaseModel):
	...
	def forward(self):
		...
		if self.opt.scale == 1:
			...
			self.dslr_warp = F.interpolate(input=self.dslr_warp, size=(2*H, 2*W), 
								         mode='bilinear', align_corners=True)
			self.dslr_mask = F.interpolate(input=self.dslr_mask, size=(2*H, 2*W), 
							             mode='bilinear', align_corners=True)
		...
		if self.isTrain:
			...
			flow = self.get_flow(F.interpolate(input=self.data_out, size=(2*H,2*W), mode='bilinear', align_corners=True), 
								 F.interpolate(input=self.data_dslr, size=(2*H,2*W), mode='bilinear', align_corners=True), 
								 self.netPWCNET)
			up_flow = F.interpolate(input=flow, size=(4*H, 4*W), mode='bilinear', align_corners=True) * 2
			self.dslr_warp, self.dslr_mask = self.get_backwarp('/', self.data_dslr, self.netPWCNET, up_flow) 
		
			if self.opt.scale == 1 or self.opt.scale == 2:
				self.data_out = F.interpolate(input=self.data_out, size=(4*H, 4*W), 
											mode='bilinear', align_corners=True)
	...
	def backward(self):  
		...
		self.loss_Total = self.loss_GCMModel_L1 + self.loss_SRResNet_L1 + \
		                  self.loss_SRResNet_VGG + self.loss_SRResNet_SSIM * 0.15
